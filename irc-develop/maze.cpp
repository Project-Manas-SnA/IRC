...
int main(){
	...
	while(1)
	{
		...
		file.close();
		Json::Value sensors;
        sensors["encoder"]["x"] = distx;
        sensors["encoder"]["y"] = disty;
        sensors["us_front"] = ultrasonicFront();
        sensors["us_left"] = ultrasonicLeft();
        sensors["us_right"] = ultrasonicRight();
        sensors["theta"] = Direction;
        fstream file;
        file.open("sensors.json",ios::out|ios::trunc);
        file<<sensors;
        file.close();
	}
	return 0;
