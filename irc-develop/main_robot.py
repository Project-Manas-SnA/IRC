...
class IRC:
    ...
    def sendControl(self,control):
        file = open("control.txt", 'rw+')
        file.write("1")
        file.write(str(control))
        file.close()

    def getRobotX(self):
        with open("sensors.json") as f:
            data = json.load(f)
        return data["encoder"]["x"]

    def getRobotY(self):
        with open("sensors.json") as f:
            data = json.load(f)
        return data["encoder"]["y"]
    ...
    def getRight(self):
        with open("sensors.json") as f:
            data = json.load(f)
        return data["us_right"]

    def turnleft(self):
        self.theta = (self.theta + 1) % 4
        while not theta == self.getTheta():
            # robotTurnLeft()
            self.sendControl(1)
        self.forward()
        # self.forward()
